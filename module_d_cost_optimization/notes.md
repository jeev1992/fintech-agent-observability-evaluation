...
## Why Cost Matters for Multi-Agent Systems

Every customer query generates **at least 2 LLM calls** (supervisor + agent). Policy queries generate 3+ calls (supervisor + retriever embedding + LLM with context).

This adds up fast:

```
1,000 queries/day × 2 LLM calls × $0.002/call = $4/day = $1,460/year
```

At scale, cost optimization is the difference between a viable product and burning cash.
...
| Token Type | GPT-4o-mini Price | GPT-4o Price |
|-----------|------------------|--------------|
| Input (prompt) | $0.15 / 1M tokens | $2.50 / 1M tokens |
| Output (completion) | $0.60 / 1M tokens | $10.00 / 1M tokens |

**Output tokens cost 2–8x more** than input tokens. Always optimize output length first.

### Token ≠ Word

- ~1 token ≈ 0.75 words (English)
- Different models use different tokenizers
- Use `tiktoken` for local token counting (OpenAI models)

---

## Multi-Agent Cost Structure

For our FinTech support agent, a single query costs:

| Component | Prompt Tokens | Completion Tokens | Notes |
|-----------|---------------|-------------------|-------|
| **Supervisor** | ~85–100 | ~3–5 | System prompt + query → single word output |
| **Policy Agent (RAG)** | ~800–1,500 | ~50–150 | System prompt + retrieved context + query |
| **Account Agent** | ~200–400 | ~50–100 | System prompt + account JSON |
| **Escalation Agent** | ~80–120 | ~80–120 | System prompt + query |

**Key insight:** The supervisor is cheap but runs on EVERY query. The policy agent is the most expensive because it includes retrieved document context.

---

## Segment 14: Token Counting & Cost Logging
...
import tiktoken
encoder = tiktoken.encoding_for_model("gpt-4o-mini")
tokens = encoder.encode("What is the overdraft fee?")
...
**System prompts count as input on EVERY call** — this is a hidden cost multiplier. A 200-token system prompt across 2 LLM calls = 400 extra tokens per query.

---

## Segment 15: Cost Optimization — Before/After

### High-Impact Optimization Patterns

| Pattern | Savings | Complexity |
|---------|---------|------------|
| **Cheaper model for supervisor** | 10–30% | Low — swap model parameter |
| **Reduce retrieval k** (5→3) | 20–40% | Low — fewer context tokens |
| **Smaller chunks** (1000→400) | 10–25% | Low — less text per chunk |
| **Shorter system prompts** | 5–15% | Low — trim unnecessary instructions |
| **Prompt caching** | 25–50% | Medium — requires provider support |
| **Semantic caching** | 15–60% | Medium — requires vector DB |
| **Model routing/cascading** | 30–60% | Medium — classifier needed |
| **Batch API** | 50% | Low — but async (24-hour window) |

### Before/After Methodology

1. **Baseline:** Run test queries with default config, measure cost
2. **Optimize:** Apply changes (cheaper model, lower k, smaller chunks)
3. **Measure:** Run same queries with optimized config
4. **Compare:** Side-by-side table of tokens, cost, quality
5. **Verify:** Check quality wasn't degraded (run Module B evaluators)

**Critical:** Always measure quality alongside cost. A system that's cheap but wrong is worthless.
...
```
Raw Agent (Module 2/support_agent.py)
  │
  ├── + Observability (Module A) → LangSmith traces on every call
  │
  ├── + Evaluation (Module B) → Datasets, MRR, DeepEval, G-Eval
  │
  ├── + Guardrails (Module C) → PII redaction, content validation
  │
  └── + Cost Optimization (Module D) → Token budgets, model routing
  │
  ▼
...
|--------|-------|-----------|
| Routing accuracy | Module B | > 95% |
| Retrieval MRR | Module B | > 0.8 |
| Faithfulness | Module B | > 0.8 |
| Correctness | Module B | > 0.8 |
| Empathy (G-Eval) | Module B | > 0.7 |
| PII leak rate | Module C | 0% |
| Cost per query | Module D | Within budget |

### This Is Ongoing

Production hardening = ongoing:
- Continuous trace review
- Evaluation in CI/CD (pytest + DeepEval)
...
|---------|----------|---------|
| 14. Token Counting & Cost Logging | 10 min | tiktoken, get_openai_callback, per-call cost |
| 15. Before/After Cost Comparison | 15 min | Baseline vs optimized, side-by-side table |
| 16. Wrap-Up | 5 min | Architecture recap, metrics summary |
