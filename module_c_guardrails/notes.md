...
## Guardrails: Middleware for LLMs

Observability shows *what happened*; guardrails *prevent bad things from happening*.

Think of guardrails as **middleware for LLMs** — they intercept and validate before output reaches the user.
...
|----------|-----------|-----------|
| **Prompt instructions** | "Don't reveal SSNs" in system prompt | Suggests behavior — LLM may ignore |
| **Guardrails** | Code that validates input/output programmatically | Enforces behavior — deterministic |

**Always use both.** Prompts for guidance, guardrails for enforcement.

---

## Guardrail Complexity Spectrum

Use the lightest check that works:

```
Regex (fastest, cheapest)
  → Schema validation (Pydantic)
    → LLM-based classification (most flexible, slowest)
```

Each layer adds latency and cost. Start with regex; add LLM-based only when regex/schema aren't sufficient.

---

## Segment 11: Motivating Demo

**The problem:** Our FinTech agent has access to mock account data including sensitive information (SSNs, balances). Without guardrails:

- A prompt injection could make it reveal SSN data
- The LLM could hallucinate financial advice not in our policies
- Toxic or manipulative inputs could produce harmful responses

**Before guardrail:** Agent leaks sensitive account data.
**After guardrail:** Same call blocked, user gets a safe response.

---

## Segment 12: Guardrails AI Library

[Guardrails AI](https://www.guardrailsai.com/) provides a framework with 50+ pre-built validators from the Guardrails Hub.

### Key Validators We Use

| Validator | Type | What It Does |
|-----------|------|-------------|
| `RegexMatch` | Regex | Blocks strings matching patterns (SSN, credit card numbers) |
| `ToxicLanguage` | LLM-based | Detects toxic, offensive, or harmful content |
| `CompetitorCheck` | LLM-based | Flags mentions of competitor brands |
| Pydantic schema | Schema | Ensures output has required fields/types |
...
guard = Guard().use_many(
    RegexMatch(regex=r"\b\d{3}-\d{2}-\d{4}\b", on_fail="fix"),  # SSN pattern
    ToxicLanguage(on_fail="exception"),
    CompetitorCheck(competitors=["Chase", "Wells Fargo", "Citi"], on_fail="fix"),
)

result = guard.validate(agent_output)
```

**Key insight:** Guardrails AI re-asks the LLM on failure (configurable). Schema guardrails validate structure, not semantic correctness.

---

## Segment 13: GDPR/HIPAA Engineering Patterns

### Microsoft Presidio for PII Detection

[Presidio](https://microsoft.github.io/presidio/) detects and redacts PII:
...
# Redact PII
anonymized = anonymizer.anonymize(text="My SSN is 123-45-6789", analyzer_results=results)
# Output: "My SSN is <PERSON>" or "My SSN is <US_SSN>"
```
...
| Tool | Best For |
|------|----------|
| **Regex** | Known patterns (SSN, credit card, phone) — fast, deterministic |
| **Presidio** | Broad PII detection (names, addresses, emails, medical IDs) — ML-based |
| **Guardrails AI** | Output validation with re-ask capability |

### Compliance Engineering Patterns

- **PII redaction** — Remove PII from inputs BEFORE they reach the LLM
- **Data minimization** — Only send necessary context to the LLM
- **Session isolation** — Don't persist conversation data across users
- **DPA for GDPR** — Data Processing Agreement required with LLM provider
- **BAA for HIPAA** — Business Associate Agreement required

> Most engineers don't realize: sending PII to LLM APIs requires a DPA (GDPR) or BAA (HIPAA) with the provider.

---

## Safe Pipeline Architecture

```
Query
  → Input Guard (Presidio PII redaction + toxicity check)
    → Multi-Agent Graph (supervisor → agent)
      → Output Guard (Guardrails AI validators)
        → User
```

Catches unsafe input before reaching agents; catches ungrounded claims and PII leaks before the user sees them.
...
|---------|----------|---------|
| 11. Guardrails Concept | 10 min | Demo: PII leak before/after |
| 12. Guardrails AI Library | 25 min | Hands-on: RegexMatch, Pydantic, ToxicLanguage, CompetitorCheck |
| 13. GDPR/HIPAA Patterns | 15 min | Hands-on: Presidio PII redaction, compliance patterns |
